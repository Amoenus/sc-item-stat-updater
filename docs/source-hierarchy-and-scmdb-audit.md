...
4. Mining element residue (refinery hints).
   DataCore now supplies material name, rarity, asteroid/surface/ground/FPS scan signatures from `mining-rock-signatures.datacore.csv`, and quality bands from `mining-quality-quantizations.datacore.csv`. SCMDB cannot create active mining-element target rows and no longer backfills mining behavior, rarity, density, scan signatures, or quality bands. With the checked-in DataCore/SCMDB data, the active merged rows are now `DataCore` or `DataCore+SCMDB`, never `SCMDB` only. The remaining SCMDB bridge field on targetable merged element rows is the derived best-refinery hint. Replace it with a first-party join if a station/material bonus source is found before retiring the SCMDB mining-elements bridge entirely.
