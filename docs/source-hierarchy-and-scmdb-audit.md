...
1. Mission titles/descriptions/chains/rewards.
   Build a first-party mission/contract extractor and reproduce SCMDB contract metadata joins from DataCore records. DataCore now emits `contract-generators.datacore.csv` with 496 generated contract variant rows, `mission-brokers.datacore.csv` with 2,584 first-party MissionBrokerEntry rows, and `mission-localization.datacore.csv` with 5,087 mission/contract localization-reference rows and 1,027 unique keys. With checked-in DataCore `4.8.0.11875683-live` and SCMDB `4.8.1-live.11875683`, combined generator/broker/localization keys overlap 476 of 1,449 SCMDB mission rows: 238/694 title rows and 228/721 description rows. Treat them as diagnostic/reconstruction inputs, not active replacements for `missions/scmdb-missions.csv`.
