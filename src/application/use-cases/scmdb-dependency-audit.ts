...
> = {
  ...
  'mission-scmdb-descriptions': {
    classification: 'Probably extractable from DataCore with new graph traversal',
    reason:
      'Mission descriptions are currently sourced from SCMDB contracts enriched with cooldowns, encounters, hauling details, blueprint rewards, and item reward joins. DataCore now emits contract-generators.datacore.csv for generated title/description variants, contract-templates.datacore.csv for objective/detail keys and display flags, mission-brokers.datacore.csv for first-party broker rewards/timing/flags, mission-contract-intel.datacore.csv for DataCore-derived reward/time/cooldown text, and mission-localization.datacore.csv from mission/contract record graph references. Combined checked-in DataCore keys overlap 227 of 720 SCMDB description rows; contract-intel rows overlap 26 SCMDB ContractIntel keys and exactly match 8 cooldown strings, so these are reconstruction inputs rather than active replacements.',
    migrationSlice:
