...
import { buildPatchData } from './updater.js';
 ...
 */
export async function generateArtifact(categories, opts) {
  const entries = {};
  const issues = [];
  let totalSkipped = 0;
  let totalErrors = 0;
 ...
 */
export async function writeArtifactFile(artifactPath, artifact) {
 ...
 */
export async function readArtifactFile(artifactPath) {
  let raw;
  try {
