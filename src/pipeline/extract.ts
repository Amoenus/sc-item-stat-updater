...
// src/pipeline/ -> src/ -> LIVE/ (it was bin/ -> english/ -> Localization/ -> Data/ -> LIVE/, which is 4 up)
// Original bin/extract-global-ini.ts had:
// const LIVE_DIR = path.resolve(__dirname, '..', '..', '..', '..');
// So `bin` is root/bin.
// Now `__dirname` is `root/src/pipeline`.
// To get to `LIVE`, from `bin` it goes `..` (root) then `..` (parent of root) then `..` then `..`
const LIVE_DIR = path.resolve(__dirname, '..', '..', '..', '..', '..');
